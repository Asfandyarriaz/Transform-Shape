...
public class ParticleEffectsLibrary : MonoBehaviour {
	...
	void Awake () {
		...
		if (ParticleEffectSpawnOffsets.Length != TotalEffects) {
			////Error ("ParticleEffectsLibrary-ParticleEffectSpawnOffset: Not all arrays match length, double check counts.");
		}
		if (ParticleEffectPrefabs.Length != TotalEffects) {
			////Error ("ParticleEffectsLibrary-ParticleEffectPrefabs: Not all arrays match length, double check counts.");
		}
